...
class _TootCardState extends State<TootCard> 
{
  ...
  void initState() 
  {
    ...
    {
      isReblogged = true;
      reblogAccount = PleromaAccount.fromJson(widget.postInfo.getReblogContent["account"]);
    }
    ...
    else if (widget.postInfo.getCardInformation != null && widget.postInfo.getCardInformation!.isNotEmpty)
    {
    ...
    String content = widget.postInfo.getContent.replaceAll("</p><p>","<br/><br/>").replaceAll("<p>", "").replaceAll("</p>", "");
  ...
  Widget build(BuildContext context) 
  {
    return ListTile(
        leading:  CircleAvatar(
          backgroundImage: NetworkImage( isReblogged ? (reblogAccount.staticAvatar ?? "") : (widget.postInfo.getAccount.staticAvatar ?? '')),
        ...
        subtitle: Column(
          ...
          children: <Widget>
          [
            Text("${ isReblogged ? reblogAccount.displayName : widget.postInfo.getAccount.displayName }"
            ...
            Wrap(
              ...
              children: <Widget>
              [
                _buildIconLabelButton("${widget.postInfo.getReplyCount}",Icons.chat_bubble_outlined),
                _buildIconLabelButton("${widget.postInfo.getReblogCount}",Icons.repeat_rounded),
                _buildIconLabelButton("${widget.postInfo.getFavouritedCount}",Icons.thumb_up_alt_rounded),
              ],
            ),
...
/* --- BUILD WIDGETS --- */
  Widget _buildRetootCard()
  {
    ...
    return ListTile(
        leading:  CircleAvatar(
          backgroundImage: NetworkImage( quoteRetootAccount["image"]),
          backgroundColor: const Color.fromARGB(70, 0, 0, 0),
          ),
      ...
      replies.add(
        RichText(
          text: TextSpan(
            ...
            [
              TextSpan(          
                text:mentionedAccounts.length < 3 ? mentionedAccounts.join(",").replaceAll(",", " and ") : mentionedAccounts[0]+" and "+mentionedAccounts[1]+" and others",
    ...
    if(curMedia["type"] != null)
    {
      switch(curMedia["type"])
      {
        ...
        case "image":
          ...
          {
            mediaWidgets.add(SizedBox(width: curMedia["width"], height:curMedia["height"] ,child:NetworkImagePreview(actualImage: curMedia["url"], previewImage: curMedia["preview_url"],)));
          }
          else
          {
